	...
	$("#medecin").on('change',function(){
		if($(this).val()!=''){
			var datas = {
				...
				"medecin" : $(this).val(),
				"infirmier" : "<?= isset($consultation) ? $consultation['infirmier'] : '' ?>",
				"etat" : 'En attente',
			}
	...
	$("#infirmier").on('change',function(){
		if($(this).val()!=''){
			var datas = {
				...
				"pvv" : "<?= $infos['id'] ?>",
				"agent" : "<?= isset($consultation) ? $consultation['agent'] : '' ?>",
				"medecin" : "<?= isset($consultation) ? $consultation['medecin'] : '' ?>",
				"infirmier" : $(this).val(),
				"etat" : 'En attente',
			}
			...
			<?php if(isset($constantes) && !empty($constantes)){ ?>
				<div class="col s12">
					<div style="border-bottom:1px solid #eee;padding-bottom:20px;">
						Temp&eacute;rature: <?php echo $constantes['temp']; ?> &nbsp;&nbsp;&nbsp;
						Poids: <?php echo $constantes['poids']; ?>&nbsp;&nbsp;&nbsp;
						Tension: <?php echo $constantes['tension']; ?>
					</div>
				</div>
			<?php }else{ ?>
		...
		<?php }elseif($this->ion_auth->in_group(array('agent'))){ ?>
			...
			$infirmiers = get_infirmiers(); //var_dump($medecins); exit;
				...
				foreach($infirmiers as $kk => $vv){
					echo '<option ';
					echo ($consultation['infirmier']==$kk) ? 'selected' : '';
					echo ' class="option'.$kk.'" value="'.$kk.'">'.$vv.'</option>';
				}
				echo '</select>';
			}
