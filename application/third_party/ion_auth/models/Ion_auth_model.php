...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $group, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', entityid, email, id, password, active, last_login, company, last_name, first_name')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
...
echo '<br> ______________________ <br>';*/
		...
		if ($query->num_rows() === 1)
		{
			...
			$group_Objs = $this->get_users_groups($user->id)->result();
			$groups = array();
			foreach ($group_Objs as $grp):
				array_push($groups, $grp->name);
			endforeach;

			//if(!in_array($group, $groups)) return FALSE;
			...
			if ($password === TRUE)
			{
				...
				if($this->input->post('api')!==null){
					unset($user->password);
					$user->groups = $this->get_users_groups($user->id)->result();
	...
	public function is_time_locked_out($identity) {
		/*if($this->is_max_login_attempts_exceeded($identity) && $this->get_last_attempt_time($identity) > time() - $this->config->item('lockout_time', 'ion_auth')){
			echo 'true'; exit;
		}
		else{
			echo 'false'; exit;
