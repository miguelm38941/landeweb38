...
var_dump($device); exit;*/
		...
		elseif($action=='getac'){
			if (isset($_GET['vc']) && !empty($_GET['vc'])) {
				$data = $this->getDeviceAcSn($_GET['vc']);
	...
	public function process_register($RegTemp){
		...
		if (strtoupper($vStamp) == strtoupper($salt)) {
			...
			if ($fid == 0) {
				...
				$res['result'] = false;
				$res['user_finger_'.$user_id] = "Ce template existe d&eacute;j&agrave;.";
			}
	...
	function getUserFingerByUserId($user_id) {
		//$pvv = $this->lg->get_data('pvv',array('code' => $id2),true);
		$user = $this->lg->get_user($user_id);
		
		$finger = $this->lg->get_finger($user['username']);
